...
namespace Pixyz.ImportSDK {
    ...
    public class ImportSettingsEditor : UnityEditor.Editor {
        ...
        private static void Register() {
            ...
            // Transforms
            templatePC.stitchPatches.status = ParameterAvailability.Hidden;
            templatePC.stitchPatches.defaultValue = false;
            templatePC.treeProcess.status = ParameterAvailability.Hidden;
            templatePC.treeProcess.defaultValue = TreeProcessType.FULL;
            templatePC.scaleFactor.status = ParameterAvailability.Hidden;
            templatePC.scaleFactor.defaultValue = 0.001f;
            // Geometry
            ...
            templatePC.avoidNegativeScale.status = ParameterAvailability.Hidden;
            templatePC.avoidNegativeScale.defaultValue = false;
        ...
        public void drawGUI(ImportSettingsTemplate template) {
             ...
             || template.treeProcess.status != ParameterAvailability.Hidden)
                {
                    EditorGUIExtensions.DrawSubCategory(Style, "TRANSFORMS", () => {
                        ...
                        drawSetting(ref importSettings.orientation, template.orientation);
                        if (importSettings.orientation == Orientation.MANUAL)
                        {
                            drawSetting(ref importSettings.isLeftHanded, template.isLeftHanded);
                            drawSetting(ref importSettings.isZUp, template.isZUp);
                        }
                        drawSetting(ref importSettings.stitchPatches, template.stitchPatches);
                        drawSetting(ref importSettings.treeProcess, template.treeProcess);
                        switch (importSettings.treeProcess)
                        {
                            case TreeProcessType.MERGE_BY_HIERARCHY_LEVEL:
                                drawSetting(ref importSettings.treeLevel, template.treeLevel);
                                break;
                            case TreeProcessType.MERGE_BY_REGIONS:
                                drawSetting(ref importSettings.mergeBy, template.mergeByRegionsStrategy);
                                if (importSettings.mergeBy == MergeByRegions.NUMBER_OF_REGIONS)
                                {
                                    drawSetting(ref importSettings.numberOfRegions, template.numberOfRegions);
                                } 
                                else if (importSettings.mergeBy == MergeByRegions.SIZE_OF_REGIONS)
                                {
                                    drawSetting(ref importSettings.sizeOfRegions, template.sizeOfRegions);
                                }
                                drawSetting(ref importSettings.mergeType, template.mergeType);
                                break;
                            default:
                                break;
                        }
            ...
            if (template.splitTo16BytesIndex.status != ParameterAvailability.Hidden
             ...
             || template.avoidNegativeScale.status != ParameterAvailability.Hidden
             ...
             || template.qualities.status != ParameterAvailability.Hidden) {
                EditorGUIExtensions.DrawSubCategory(Style, "GEOMETRY", () => {
                    ...
                    drawSetting(ref importSettings.avoidNegativeScale, template.avoidNegativeScale);
                    ...
                    if (IsDrawingForPointCloud) {
                        ...
                        {
                            MeshQuality meshQuality = (MeshQuality)(int)importSettings.qualities.quality.quality;
                            if (meshQuality > MeshQuality.CUSTOM)
                                meshQuality = MeshQuality.CUSTOM;
                            drawSetting(ref meshQuality, template.quality);
                            var lodSettings = new LodGenerationSettings();
                            lodSettings.quality = (LodQuality)(int)meshQuality;
                            lodSettings.threshold = importSettings.qualities.quality.threshold;
                            ...
                            if (meshQuality == MeshQuality.CUSTOM)
                            {
                                EditorGUILayout.LabelField("Custom your parameter through Edit > Preferences > Pixyz", EditorStyles.miniLabel);
                                lodSettings.quality = LodQuality.CUSTOM;
                                qualities.quality = lodSettings;
                            }
                            importSettings.qualities = qualities;
