...
namespace Pixyz.ImportSDK
{
    public class PostprocessVRED : SubProcess {
        ...
        private void mergeAnimations(ImportStamp importedModel, string animationPath) {
            ...
            importedModel.transform.rotation = Quaternion.identity;
            SceneExtensionsEditor.SetParentSafe(animationRoot, pxzRoot.transform.parent, false);
        ...
        private void merge(Transform animationRoot, Transform pxzRoot)
        {
            if (pxzRoot.GetComponent<Metadata>())
            {
                ...
                var nprop = new Properties();
                nprop.values = new Core.Native.StringList(0);
                nprop.names = new Core.Native.StringList(0);
