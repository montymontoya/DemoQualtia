...
namespace Pixyz.ImportSDK
{
    ...
    {
        #region Time
        public static DateTime ToUnityObject(this Core.Native.Date date)
        {
            return new DateTime(date.year, date.month, date.day);
        }

        public static string ToEndDateRichText(this Core.Native.Date date)
        {
        ...
        public static string GetRemainingDaysText(this Core.Native.Date date)
        {
            var validity = new DateTime(date.year, date.month, date.day);
            int daysRemaining = Math.Max(0, (validity - DateTime.Now).Days + 1);
            if (daysRemaining == 0)
