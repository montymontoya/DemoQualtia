...
namespace Pixyz.Toolbox.Editor
{
    public abstract class PixyzFunction : ActionInOut<IList<GameObject>, IList<GameObject>>
    {
        ...
        protected void DeleteAllInput()
        {
            ...
            {
                SceneExtensionsEditor.DestroyImmediateSafe(go);
            }
        ...
        {
            var highestAncestors = _input.GetHighestAncestors();
            foreach (var highestAncestor in highestAncestors)
            {
                highestAncestor.SetActive(false);
            }
            _input = CleanList(_input);
        }
