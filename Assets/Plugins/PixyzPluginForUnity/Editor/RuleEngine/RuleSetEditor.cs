...
namespace Pixyz.RuleEngine.Editor
{
    ...
    public sealed class RuleSetEditor : UnityEditor.Editor {
        ...
        private string _currentMessage = "Processing..";
        ...
        public override void OnInspectorGUI() {
            ...
            // Rules
            isValid = true;
            EditorGUI.BeginDisabledGroup(rules.IsRunning);
            ...
            if(!isValid)
            {
                runLabel += " (Please fix errors !)";
            }
            else if(rules.IsRunning)
            {
                runLabel = $"Running... {_currentProgress * 100.0f:F}%";
            }

            if (GUILayout.Button(runLabel, GUILayout.Height(35)))
            {
                _currentMessage = "Processing..";
                _currentProgress = 0;
                rules.OnCompleted += RunCompleted;
                rules.progressed += RunProgressed;
                rules.run();
            }

            if (rules.IsRunning)
                EditorGUILayout.LabelField($"Current : {_currentMessage}");
        ...
        private void RunProgressed(float progress, string message)
        {
            ...
            if (EditorGUIExtensions.DrawButton(new Rect(rect.x + rect.width - 30, rect.y + 6, 24, 24), StaticStyles.IconDetailRuleBlock)) {
                ...
                foreach (ActionBase action in actionsForInsertBefore.OrderBy(x => x.menuPathRuleEngine)) {
                    ...
                    menu.AddItem(new GUIContent("Insert After/" + action.menuPathRuleEngine, action.tooltip), false, new GenericMenu.MenuFunction(() => {
                        block.rule.insertBlock(new RuleBlock(action.id), block.rule.getBlockIndex(block) + 1);
                    }));
                }
                // Remove
