...
namespace Pixyz.LODTools.Editor
{
    ...
    {
        [SerializeField]
        private bool _useSelectionAsOne = false;
        ...
        public void OnGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("Metadata of your selected GameObjects and their children won't be transferred to your newly GameObjects created", MessageType.Warning);
            }
        ...
        private void GenerateLOD(LODPreBuild preBuild)
        {
            ...
            try
            {
                ...
                data.materials = new List<UnityEngine.Material[]>(renderers.Count);
                ...
                Core.Native.NativeInterface.PushAnalytic("LOD Generation", "");
        ...
        private void GenerationCompleted(bool finishWithoutErrors)
        {
            ...
            try
            {
                ...
                if (preBuild.pastGeneration == null)
                {
                    if(_useSelectionAsOne)
                    {
                        generationName = "CustomGroup_PixyzLOD";
                    }
                    else
                    {
                        generationName = preBuild.gos[0] != null ? preBuild.gos[0].name + "_PixyzLOD" : "PixyzLOD";
                    }
                    GameObject newLodGroupGameobject = new GameObject(generationName);
                ...
                if(_useSelectionAsOne)
                {
                    for (int i =0;i<preBuild.builder.Contexts.Length;i++)
                    {
                        if(i > 0)
                        if (preBuild.buildProcess.Rules[i-1].isCombineMaterialsActivated || preBuild.buildProcess.Rules[i-1].isCombineMaterialsActivated)
                            continue;
                        GameObject newRoot = new GameObject($"CustomGroup_PixyzLOD{i}");
                        newRoot.transform.parent = preBuild.pastGeneration.transform;
                ...
                foreach (Renderer r in preBuild.srcRenderers)
                {
                    ...
                    string name = $"{r.gameObject.name}_LOD0";
                    GameObject copiedGameobject = new GameObject(name);
                    ...
                    if (_useSelectionAsOne)
                    {
                        copiedGameobject.transform.parent = rootGameObjects[0].transform;
                    }
                    else
                    {
                        GameObject rootGameObject = new GameObject($"{r.gameObject.name}");
                        rootGameObject.transform.parent = preBuild.pastGeneration.transform;

                        var metadata = r.GetComponent<Pixyz.ImportSDK.Metadata>();
                        if (metadata != null)
                        {
                            UnityEditorInternal.ComponentUtility.CopyComponent(metadata);
                            UnityEditorInternal.ComponentUtility.PasteComponentAsNew(rootGameObject);
                        }

                        copiedGameobject.transform.parent = rootGameObject.transform;
                ...
                for (int i = 1; i < preBuild.builder.Contexts.Length; ++i)
                {
                    ...
                    for (int j = 0; j < generatedRenderer.Length; ++j)
                    {
                        ...
                        string ruleName = "";
                        if (preBuild.buildProcess.Rules[i - 1].name != "")
                            ruleName = preBuild.buildProcess.Rules[i - 1].name;
                        else
                            ruleName = $"LODRule{i}";

                        bool combined = preBuild.buildProcess.Rules[i - 1].isRemeshActivated || preBuild.buildProcess.Rules[i -1].isRemeshFieldAlignedActivated || preBuild.buildProcess.Rules[i - 1].isCombineActivated || preBuild.buildProcess.Rules[i - 1].isCombineMeshesByMaterialsActivated || preBuild.buildProcess.Rules[i-1].isBillboardActivated || preBuild.buildProcess.Rules[i-1].isImpostorActivated;
                        if (combined)
                        {
                            if(_useSelectionAsOne)
                                r.gameObject.name = $"CustomGroup_{ruleName}_L0D{i}";
                            else
                                r.gameObject.name = $"{preBuild.gos[0].name}_{ruleName}_L0D{i}";

                        }
                        else
                        {
                            if (preBuild.buildProcess.Rules[i - 1].name != "")
                                r.gameObject.name = $"{data.renderers[j].name}_{ruleName}_L0D{i}";
                            else
                                r.gameObject.name = $"{data.renderers[j].name}_{ruleName}_L0D{i}";
                        }
                        ...
                        r.transform.position = r.transform.position == Vector3.zero ? pos : r.transform.position;
                        r.transform.rotation = r.transform.rotation == Quaternion.identity ? rot : r.transform.rotation;
                        r.transform.localScale = r.transform.localScale == Vector3.one ? scale : r.transform.localScale;
                        ...
                        if(_useSelectionAsOne)
                        {
                            if (combined)
                                ...
                                r.transform.parent = rootGameObjects[i].transform;
                        }
                        else
                        {
                            if (combined)
                                ...
                                r.transform.parent = rootGameObjects[j].transform;
                        }

                        preBuild.builder.StartPostProcess(preBuild.buildProcess.Rules[i - 1], context, preBuild.gos[0], r, i);
                ...
                for (int n = 0;n<preBuild.gos.Count;n++)
                {
                    if (preBuild.gos[n] != null)
                        preBuild.gos[n].SetActive(false);
                }
