...
namespace Pixyz.OptimizeSDK.Editor.MeshProcessing
{
	...
	{
		public Polygonal.Native.MeshList pixyzMeshes;
		public Geom.Native.Matrix4List pixyzMatrices;
		...
		/// </summary>
		private Dictionary<uint, UnityEngine.Material[]> _unityMaterialsLookUp = null;

		/// <summary>
		/// Keeps track of mesh names
		/// </summary>
		private Dictionary<uint, string> _unityMeshNamesLookUp = new Dictionary<uint, string>();

		/// <summary>
		/// Default line material to render lines
		/// </summary>
		private UnityEngine.Material _lineMaterial = null;
		...
		private UnityEngine.Material LineMaterial
		{
			get
			{
				...
				{
					_lineMaterial = new UnityEngine.Material(ShaderUtilities.GetPixyzLineShader());
				}
		...
		/// </summary>
		private UnityEngine.Material _pointMaterial = null;

		/// <summary>
		/// Default Point material to render point cloud
		/// </summary>
		private UnityEngine.Material PointMaterial
		{
			get
			{
				if (_pointMaterial == null)
				{
					_pointMaterial = new UnityEngine.Material(ShaderUtilities.GetPixyzSplatsShader());
				}
		...
		/// </summary>
		private UnityEngine.Material _missingMaterial = null;

		/// <summary>
		/// Default material used when a material was not found
		/// </summary>
		private UnityEngine.Material MissingMaterial
		{
            get
			{
				if(!_missingMaterial)
                {
					_missingMaterial = new UnityEngine.Material(ShaderUtilities.GetDefaultShader());
				...
				case MaterialSyncType.SyncFull:
					{
						...
						_materialPixyzToUnity = new Dictionary<uint, UnityEngine.Material>();
						_materialUnityToPixyz = new Dictionary<UnityEngine.Material, uint>();
					}
					break;
				...
				case MaterialSyncType.SyncSendOnly:
					{
						_imagesUnityToPixyz = new Dictionary<Texture2D, uint>();
						_materialUnityToPixyz = new Dictionary<UnityEngine.Material, uint>();
						_unityMaterialsLookUp = new Dictionary<uint, UnityEngine.Material[]>();
					}
					break;
            }
        }
		...
		public uint[] UnityMeshesToPixyzCreate(in MeshData meshData)
		{
			...
			Polygonal.Native.MeshDefinition[] defs = new Polygonal.Native.MeshDefinition[count];
			pixyzMatrices = new Geom.Native.Matrix4List(count);

			for (int i = 0; i < filteredData.renderers.Count; ++i)
			{
				defs[i] = UnityMeshToPixyz(filteredData.renderers[i], filteredData.meshes[i], filteredData.materials[i], (uint)i);
				_unityMeshNamesLookUp.Add((uint)i, filteredData.meshes[i].name);
			}

			pixyzMeshes = Polygonal.Native.NativeInterface.CreateMeshesFromDefinitions(new Polygonal.Native.MeshDefinitionList(defs));

			Polygonal.Native.MeshList meshList = new Polygonal.Native.MeshList(outRenderers.Count);
			Geom.Native.Matrix4List matrixList = new Geom.Native.Matrix4List(outRenderers.Count);
		...
		{
			Polygonal.Native.MeshDefinition def = UnityMeshToPixyz(renderer, mesh, materials);
			def.id = Polygonal.Native.NativeInterface.CreateMeshFromDefinition(def);

			pixyzMatrices = new Geom.Native.Matrix4List(new Geom.Native.Matrix4[] { Conversions.ConvertMatrix(renderer.transform.localToWorldMatrix) });
			pixyzMeshes = new Polygonal.Native.MeshList(new uint[] { def.id });
		...
		public uint AddUnityMeshToPixyzCreate(Renderer renderer, Mesh mesh, UnityEngine.Material[] materials)
		{
			...
			Geom.Native.Matrix4List matriceList = new Geom.Native.Matrix4List(pixyzMatrices.length + 1);
			...
			Polygonal.Native.MeshList meshList = new Polygonal.Native.MeshList(pixyzMeshes.length + 1);
			Array.Copy(pixyzMatrices, matriceList, pixyzMatrices.length);
		...
		/// <param name="materials"></param>
		/// <returns></returns>
		public Polygonal.Native.MeshDefinition[] UnityMeshesToPixyz(IList<Renderer> renderers, IList<Mesh> meshes, IList<UnityEngine.Material[]> materials)
		{
			if (_syncMaterials != MaterialSyncType.SyncFull)
				Debug.LogWarning("[PixyzContext] This method only support MaterialSyncType.SyncFull");

			Polygonal.Native.MeshDefinition[] defs = new Polygonal.Native.MeshDefinition[renderers.Count];
		...
		/// <param name="materials"></param>
		/// <returns></returns>
		public Polygonal.Native.MeshDefinition UnityMeshToPixyz(Renderer renderer, Mesh mesh, UnityEngine.Material[] materials, uint index = 0)
		{
			if(!mesh.isReadable)
			{
			...
			Polygonal.Native.MeshDefinition def = Conversions.ConvertMesh(mesh, materialIds);
			def.externalId = index;
		...
		/// <param name="materials"></param>
		/// <returns></returns>
		public Polygonal.Native.MeshDefinition AddUnityMeshToPixyz(Renderer renderer, Mesh mesh, UnityEngine.Material[] materials)
		{
			if (_syncMaterials != MaterialSyncType.SyncFull)
				Debug.LogWarning("[PixyzContext] This method only support MaterialSyncType.SyncFull");
		...
		private Material.Native.MaterialList GenerateMaterials(UnityEngine.Material[] materials)
		{
			List<Material.Native.MaterialDefinition> defs = new List<Material.Native.MaterialDefinition>();
			Material.Native.MaterialList materialsPixyz = new Material.Native.MaterialList(materials.Length);
			...
			for (int i = 0; i < materials.Length; ++i)
			{
				...
				{
					Material.Native.MaterialDefinition def = null;
			...
			Material.Native.MaterialList list = Material.Native.NativeInterface.CreateMaterialsFromDefinitions(new Material.Native.MaterialDefinitionList(defs.ToArray()));
		...
		{
			List<Material.Native.ImageDefinition> defs = new List<Material.Native.ImageDefinition>();
			...
			for(int i =0; i < textures.Keys.Count; ++i)
			{
				...
				{
					Material.Native.ImageDefinition def = Conversions.ConvertTexture(tex, texturesTypes[tex] == "_BumpMap");
					defs.Add(def);
			...
			Material.Native.ImageList list = Material.Native.NativeInterface.CreateImagesFromDefinitions(new Material.Native.ImageDefinitionList(defs.ToArray()));
		...
		private void ApplyCoeff(Material.Native.ImageDefinition image, Color coef)
		{
			...
			if (coef.a != 1.0f && image.componentsCount != 4)
			{
				...
				image.data = new Core.Native.ByteList(4 * image.height * image.width);
			}
		...
		private Polygonal.Native.JointList GenerateBones(SkinnedMeshRenderer renderer)
		{
			if(renderer == null)
			{
				return new Polygonal.Native.JointList();
			}

			Polygonal.Native.JointList joints = new Polygonal.Native.JointList(renderer.bones.Length);
			...
			Polygonal.Native.PlaceholderJointList list = Polygonal.Native.NativeInterface.CreateJointPlaceholders(new Core.Native.ULongList(transformAddIds.ToArray()), Conversions.ConvertMatrices(transformAddMatrices));
		...
		{
            Polygonal.Native.MeshDefinition[] meshDefinitions = Polygonal.Native.NativeInterface.GetMeshDefinitions(new Polygonal.Native.MeshList(meshIds));
            GameObject[] gameObjects = new GameObject[meshDefinitions.Length];

			string[] names = Core.Native.NativeInterface.GetProperties(new Core.Native.EntityList(meshIds), "ExternalId", "Pixyz_Mesh");
		...
		/// <param name="meshIds"></param>
		/// <returns></returns>
		public GameObject[] PixyzMeshToUnityObject(IEnumerable<Polygonal.Native.MeshDefinition> sourceDefs)
		{
			uint[] meshIds = sourceDefs.Select(d => d.id).ToArray();

			var meshes = new Core.Native.EntityList(meshIds);
			Polygonal.Native.MeshDefinition[] meshDefinitions = Polygonal.Native.NativeInterface.GetMeshDefinitions(new Polygonal.Native.MeshList(meshIds));
            GameObject[] gameObjects = new GameObject[meshDefinitions.Length];
			string[] names = Core.Native.NativeInterface.GetProperties(new Core.Native.EntityList(meshIds), "ExternalId", "Pixyz_Mesh");
		...
		/// <param name="meshIds"></param>
		/// <returns></returns>
		public GameObject PixyzMeshToUnityObject(Polygonal.Native.MeshDefinition def)
		{
		...
		/// <param name="meshIds"></param>
		/// <returns></returns>
		public Renderer PixyzMeshToUnityRenderer(Polygonal.Native.MeshDefinition def)
		{
		...
		private UnityEngine.Material[] RetrieveMaterialsFromUnity(Polygonal.Native.MeshDefinition meshDefinition)
        {
			UnityEngine.Material[] materials = new UnityEngine.Material[meshDefinition.dressedPolys.length + meshDefinition.lines.length + (meshDefinition.points.length > 0 ? 1 : 0)];
		...
		private UnityEngine.Material[] RetrieveMaterialsFromPixyz(Polygonal.Native.MeshDefinition definition, Mesh mesh)
		{
			...
			{
				UnityEngine.Material[] tmp = new UnityEngine.Material[mesh.subMeshCount];
		...
		private Mesh ExtractMesh(Polygonal.Native.MeshDefinition definition)
		{
			Mesh mesh = new Mesh();

			_unityMeshNamesLookUp.TryGetValue(definition.externalId, out string name);
			mesh.name = name == null ? "Mesh" : name;
		...
		private UnityEngine.Material[] ExtractMaterials(Polygonal.Native.DressedPolyList subMesh)
		{
			...
			Material.Native.MaterialDefinitionList materialDefs = Material.Native.NativeInterface.GetMaterialDefinitions(new Material.Native.MaterialList(materialsId.ToArray()));
		...
		private void ExtractBones(SkinnedMeshRenderer renderer, Polygonal.Native.JointList joints)
		{
			Transform[] bones = new Transform[joints.length];

			Core.Native.ULongList jointsList = Polygonal.Native.NativeInterface.GetJointPlaceholders(new Polygonal.Native.PlaceholderJointList(joints.list));
		...
		public object Clone()
		{
			...
			newContext.pixyzMeshes = new Polygonal.Native.MeshList(pixyzMeshes.length);
			newContext.pixyzMatrices = new Geom.Native.Matrix4List(pixyzMatrices.length);
			...
			for (int i = 0; i < pixyzMeshes.length; ++i)
			{
				...
				{
					newContext.pixyzMeshes[i] = Core.Native.NativeInterface.CloneEntity(pixyzMeshes.list[i]);
