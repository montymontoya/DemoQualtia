...
namespace Pixyz.ImportSDK {
    ...
    public sealed class Metadata : MonoBehaviour {
        ...
        public bool removeProperty(string propertyName) {
            for (int i = 0; i < properties.names.Length; i++) {
                if (_properties.names[i] == propertyName) {
                    OptimizeSDK.Utils.CollectionExtensions.RemoveAt(ref _properties.names, i);
                    OptimizeSDK.Utils.CollectionExtensions.RemoveAt(ref _properties.values, i);
                    return true;
                }
            }
            return false;
        ...
        public void addOrSetProperty(string propertyName, string propertyValue) {
            ...
            OptimizeSDK.Utils.CollectionExtensions.Append(ref _properties.names, propertyName);
            OptimizeSDK.Utils.CollectionExtensions.Append(ref _properties.values, propertyValue);
        }
    }
}