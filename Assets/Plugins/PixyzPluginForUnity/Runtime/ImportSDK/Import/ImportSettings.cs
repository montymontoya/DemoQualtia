...
namespace Pixyz.ImportSDK {
    ...
    public sealed class ImportSettings : ScriptableObject {
        ...
        /// </summary>
        public Orientation orientation = Orientation.AUTOMATIC;
        ...
        /// </summary>
        public bool stitchPatches = false;
        ...
        /// <summary>
        /// Number: specify the number of output GameObject (or the number of regions of output GameObject)
        /// Size: specify the diagonal size of the output regions
        /// </summary>
        public MergeByRegions mergeBy = MergeByRegions.NUMBER_OF_REGIONS;

        /// <summary>
        /// Choose whether to merge GameObjects into one GameObject or merge GameObjects by materials (one GameObject per material)
        /// </summary>
        public MergeType mergeType = MergeType.MERGE_BY_MATERIAL;

        /// <summary>
        /// Specify the diagonal size of the output regions (in Unity unit)
        /// </summary>
        public float sizeOfRegions = 10f;
        ...
        /// </summary>
        public bool avoidNegativeScale = true;
        ...
        public ImportSettings Verify(ImportSettingsTemplate template)
        {
            ...
            nsettings.loadMetadata = (template.loadMetadata.status == ParameterAvailability.Available) ? loadMetadata : template.loadMetadata.defaultValue;
            ...
            nsettings.stitchPatches = (template.stitchPatches.status == ParameterAvailability.Available) ? stitchPatches : template.stitchPatches.defaultValue;
            ...
            nsettings.treeLevel = (template.treeLevel.status == ParameterAvailability.Available && nsettings.treeProcess == TreeProcessType.MERGE_BY_HIERARCHY_LEVEL) ? treeLevel : template.treeLevel.defaultValue;
            ...
            nsettings.useMaterialsInResources = (template.useMaterialsInResources.status == ParameterAvailability.Available) ? useMaterialsInResources : template.useMaterialsInResources.defaultValue;
            ...
            nsettings.avoidNegativeScale = (template.avoidNegativeScale.status == ParameterAvailability.Available) ? avoidNegativeScale : template.avoidNegativeScale.defaultValue;
            ...
            nsettings.orientation = (template.orientation.status == ParameterAvailability.Available) ? orientation : template.orientation.defaultValue;
            ...
            nsettings.numberOfRegions = (template.numberOfRegions.status == ParameterAvailability.Available) ? numberOfRegions : template.numberOfRegions.defaultValue;
            nsettings.sizeOfRegions = (template.sizeOfRegions.status == ParameterAvailability.Available) ? sizeOfRegions : template.sizeOfRegions.defaultValue;
            nsettings.mergeBy = (template.mergeByRegionsStrategy.status == ParameterAvailability.Available) ? mergeBy : template.mergeByRegionsStrategy.defaultValue;
            nsettings.mergeType = (template.mergeType.status == ParameterAvailability.Available) ? mergeType : template.mergeType.defaultValue;
            return nsettings;
        ...
        public static LodsGenerationSettings VerifyLODSettings(LodsGenerationSettings unityObject, ImportSettings settings, ImportSettingsTemplate template)
        {
            ...
            else
            {
                ...
                lods.lods[0].threshold = unityObject.lods[0].threshold;
            }
