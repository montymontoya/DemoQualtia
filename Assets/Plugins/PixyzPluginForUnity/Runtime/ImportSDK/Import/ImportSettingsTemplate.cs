...
namespace Pixyz.ImportSDK
{
    ...
    public struct ImportSettingsTemplate {
        ...
        public Parameter<Orientation> orientation;
        public Parameter<bool> isLeftHanded;
        public Parameter<bool> isZUp;
        public Parameter<bool> stitchPatches;
        public Parameter<TreeProcessType> treeProcess;
        public Parameter<int> treeLevel;
        public Parameter<MergeType> mergeType;
        public Parameter<MergeByRegions> mergeByRegionsStrategy;
        public Parameter<int> numberOfRegions;
        public Parameter<float> sizeOfRegions;
        ...
        public Parameter<bool> avoidNegativeScale;
        ...
        public static readonly ImportSettingsTemplate Default = new ImportSettingsTemplate {
            ...
            scaleFactor = new Parameter<float> { defaultValue = 0.001f, name = "Scale", tooltip = "Scale factor applied to the whole assembly. Pixyz is by design in mm and Unity by convention in m. The default value 0.001 will match your model with Unity scale system." },
            
            orientation = new Parameter<Orientation> { defaultValue = Orientation.AUTOMATIC, name = "Orientation", tooltip = "Pixyz can automatically orient your model according to its source coordinate system." },
            ...
            isZUp = new Parameter<bool> { defaultValue = false, name = "Z-up", tooltip = "Use this setting if the model was designed in a Z-up environment." },
            
            stitchPatches = new Parameter<bool> { defaultValue = false, name = "Stitch Patches", tooltip = "Merge objects on the last levels of the hierarchy. This will for instance merge brep patches contained in the same geometrical set.Use this setting to assemble together unconnected CAD surfaces." },
            ...
            treeProcess = new Parameter<TreeProcessType> { defaultValue = TreeProcessType.FULL, name = "Hierarchy", tooltip = "Use these functions to simplify the tree(or hierarchy) of your model.\n" +
                ...
                "> Clean-up intermediary nodes: compress the hierarchy by removing empty nodes, or any node containing only one sub-node.\n" +
                ...
                "> Merge by material: all objects contained in the original model sharing the same material will be merged together.\n +" +
                "> Merge Final Level: merge the last Level of imported model \n" +
                "> Merge By Hierarchy level: merge a specific Level of hierarchy from imported model \n" +
                "> Merge By Name: merge GameObjects sharing the same name \n" +
                "> Merge By Regions: merge GameObjects in the same areas"
            },
            treeLevel = new Parameter<int> { defaultValue = 1, name = "Hierarchy Level", tooltip = "Merge to this hierarchy level"},
            mergeByRegionsStrategy = new Parameter<MergeByRegions> { defaultValue = MergeByRegions.NUMBER_OF_REGIONS, name = "Type", tooltip = "Number: specify the number of output GameObject (or the number of regions of output GameObject)\nSize: specify the diagonal size of the output regions"},
            numberOfRegions = new Parameter<int> { defaultValue = 10, name = "Number of Regions", tooltip = "Specify the number of output GameObject (or the number of regions of output GameObject)"},
            sizeOfRegions = new Parameter<float> { defaultValue = 10, name = "Size of Regions", tooltip = "Specify the diagonal size of the output regions (in Unity unit)" },
            mergeType = new Parameter<MergeType> { defaultValue = MergeType.MERGE_BY_MATERIAL, name = "Merge Strategy", tooltip = "Choose whether to merge GameObjects into one GameObject or merge GameObjects by materials (one GameObject per material)" },
            hasLODs = new Parameter<bool> { defaultValue = false, name = "Create LODs", tooltip = "The LODs (Level Of Detail) is an optimization feature allowing Unity to automatically switch a model quality depending on its occupancy relatively to the screen (in %)." },
            ...
            avoidNegativeScale = new Parameter<bool> { defaultValue = true, name = "Avoid Negative Scale", tooltip = "If true, all GameObjects considered as symmetries will have their transform baked in their mesh vertices.\n" +
            ...
            repairInstances = new Parameter<bool> { defaultValue = false, name = "Repair Instances", tooltip = "If true, Pixyz will attempt to detect similar GameObjects and recreate instances of their meshes.\n" +
                "This can be used to repair instances or to simplify a model having similar parts that could be instanciated instead to reduce the number of unique meshes (reduces drawcalls, GPU memory usage and file size)."},
